    ...
    {localExercises
      ...
      .map((ex) => (
        <React.Fragment key={ex.id}>
          ...
          {/* Sets Table Rows */}
          {(ex.workout_sets || []).sort((a, b) => a.index - b.index).map((set, setIdx) => (
                    ...
                    setLocalExercises((prev) =>
                      prev.map((item) =>
                        item.id === ex.id
                          ? {
                              ...item,
                              workout_sets: item.workout_sets.map((s, sIdx) =>
                                sIdx === setIdx
                                  ? { ...s, reps: Number(e.target.value), completed: false }
                                  : s
                              ),
                            }
                          : item
                      ...
                      prev.map((item) =>
                        item.id === ex.id
                          ? {
                              ...
                              workout_sets: item.workout_sets.map((s, sIdx) =>
                                sIdx === setIdx
                                  ? { ...s, weight: Number(e.target.value), completed: false }
                                  : s
                              ),
                            }
                          : item
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (checked && !checkInputFields(set)) {
                      alert("Please ensure all fields are valid before marking as done.");
                      return;
                    }
                    setLocalExercises((prev) =>
                      prev.map((item) =>
                    ...
                    onToggle(ex.id, set.id, set.reps, set.weight, checked);
                  }}
